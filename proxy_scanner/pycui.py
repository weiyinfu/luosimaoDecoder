...
class color:
    def __init__(self, force_linux=False):
        if name == "nt" and not force_linux:
            # Windows
            self.RED = 0x04
            self.GREY = 0x08
            self.BLUE = 0x01
            ...
            def p(m, c=self.BLACK, e=True):
                if e:
                    ...
                    print("%s%s%s" % (c, m, self.BLACK), end=' ')
        self.p = p


class pycui:
    def __init__(self, force_linux_colors=True):
        self.c = color(force_linux_colors)
